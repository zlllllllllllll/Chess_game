...
//混乱扑克
void CGameLogic::RandCardData(BYTE cbCardData[], BYTE cbMaxCount,DWORD userid)//这个里面的随机要加二个用户的userid之合，不然会牌一样的
{
	//混乱准备
	...
	//初始化种子
	srand((DWORD)(GetTickCount()+userid));
...
BYTE CGameLogic::GetWeaveCard(BYTE cbWeaveKind, BYTE cbCenterCard, BYTE cbCardBuffer[4])
{
	...
	case WIK_LEFT:		//上牌操作
		{
			//设置变量
			if (BAIBAN_CARD_DATA == cbCenterCard)
			{
				cbCenterCard = m_byGodsCardData;
			}
			...
			for (int i=0; i<3; ++i)
			{
				if (m_byGodsCardData == cbCardBuffer[i])
				{
					cbCardBuffer[i] = BAIBAN_CARD_DATA;
				}
			}
			return 3;
		}
...
BYTE CGameLogic::EstimateEatCard(BYTE cbCardIndex[MAX_INDEX], BYTE cbCurrentCard)
{
	...
	// 财神不可以吃
	if (cbCurrentCard == m_byGodsCardData)
	{
		return WIK_NULL;
	}
	...
	//过滤判断
	//番子无连
	if ((cbCurrentCard>=0x31) && (BAIBAN_CARD_DATA != cbCurrentCard)) 
		return WIK_NULL;
	if ((cbCurrentCard>=0x31)&&((BAIBAN_CARD_DATA == cbCurrentCard)&&(m_byGodsCardData>=0x31))) 
		return WIK_NULL;
...
BYTE CGameLogic::AnalyseChiHuCard(BYTE cbCardIndex[MAX_INDEX], tagWeaveItem WeaveItem[], BYTE cbWeaveCount, BYTE cbCurrentCard, DWORD dwChiHuRight, tagChiHuResult & ChiHuResult)
{
	...
	//权位处理
	if ((cbCurrentCard!=0)&&(cbWeaveCount==MAX_WEAVE))
		dwChiHuRight|=CHK_QUAN_QIU_REN;

	//分析扑克
	CAnalyseItemArray AnalyseFallback;
	BYTE byGodsCardIndex = SwitchToCardIndex(m_byGodsCardData);
	BYTE byBaiBan = SwitchToCardIndex(BAIBAN_CARD_DATA);
	bool bIsBaDui = false;
	bool bIsBaDuiFallback = false;
	//OUTPUT_DEBUG_STRING(TEXT("服务端财神 0x%02X"), m_byGodsCardData);
	if (1 == cbCardIndexTemp[byGodsCardIndex])
	{
		CAnalyseItemArray AnalyseItemArrayTemp;
		BYTE cbCardIndexUser[MAX_INDEX];		
		for (int i=0; i<MAX_INDEX; ++i)
		{
			CopyMemory(cbCardIndexUser,cbCardIndexTemp,sizeof(cbCardIndexTemp));
			++cbCardIndexUser[i];
			--cbCardIndexUser[byGodsCardIndex];			
			AnalyseItemArrayTemp.RemoveAll();
			AnalyseCard(cbCardIndexUser,WeaveItem,cbWeaveCount,AnalyseItemArrayTemp);
			...
			if (!bIsBaDui || !bIsBaDuiFallback)
			{
				if (IsBaDui(cbCardIndexUser,WeaveItem,cbWeaveCount))
				{
					bIsBaDui = true;
					if (i == byBaiBan)
					{
	...
	else if (2 == cbCardIndexTemp[byGodsCardIndex])
	{
		CAnalyseItemArray AnalyseItemArrayTemp;
		BYTE cbCardIndexUser[MAX_INDEX];
		for (int i=0; i<MAX_INDEX; ++i)
	...
	else if (3 == cbCardIndexTemp[byGodsCardIndex])
	{
		CAnalyseItemArray AnalyseItemArrayTemp;
		BYTE cbCardIndexUser[MAX_INDEX];
		for (int i=0; i<MAX_INDEX; ++i)
		{
			for (int j=0; j<MAX_INDEX; ++j)
			{
				for (int h=0; h<MAX_INDEX; ++h)
				{
					...
					if ((i == byBaiBan) && (j==byBaiBan) && (h == byBaiBan))
					{
						AnalyseFallback.RemoveAll();
						AnalyseFallback.Append(AnalyseItemArrayTemp);
					}
	...
	else if (4 == cbCardIndexTemp[byGodsCardIndex])
	{
		CAnalyseItemArray AnalyseItemArrayTemp;
		BYTE cbCardIndexUser[MAX_INDEX];
		for (int i=0; i<MAX_INDEX; ++i)
	...
	else
	{
		...
		if (!bIsBaDui)
		{
			if (IsBaDui(cbCardIndexTemp,WeaveItem,cbWeaveCount))
			{
				bIsBaDui = true;
			}
		}
	...
	{
		// 三张财神，又有其他胡牌类型
		if (0x03 == cbCardIndexTemp[byGodsCardIndex])
		{
			dwChiHuKind |= CHK_SAN_GODS;
		}
		// 没有财神是硬牌
		if (0x00 == cbCardIndexTemp[byGodsCardIndex])
		{
		...
		if (0x02 == byCount)
		{
			dwChiHuKind |= CHK_SINGLE_PAI;
		}

		// 财神归位
		if (AnalyseFallback.GetCount()>0)
		{
			dwChiHuKind |= CHK_YING_PAI;
		}
